...
import { decreasePortion, addPortion, addToFavourites } from "../actions/index";
...
export default function TheRecipe() {
    ...
    const portionCounter = useSelector(state => state.portionCount)
    const state = useSelector(state => state.chosenRecipe)
    const chosenRecipe = state.chosenRecipe ? state.chosenRecipe : null;
    const { ingredients } = chosenRecipe !== null ? chosenRecipe : [];
    const favourites = useSelector(state => state.myFavourites.favorites)


    // Sätter default färg på favoritknapp. Kollar om den redan är tillagd favorit och byter då färg
    let btn = `${styles.noFav}`
    let btnText = "Lägg till i favoriter"
    favourites.forEach(favourite => {
        if (favourite.name === chosenRecipe.name) {
            btn = `${styles.addToFavourites}`
            btnText = "Favorit"
        }
    });
        ...
        return (
            <section className={styles.container}>
                ...
                <article className={styles.header}>
                    <button className={btn} onClick={() => dispatch(addToFavourites(chosenRecipe))}>{btnText}</button>
                ...
                <section className={styles.infoSection}>
                    ...
                    <img src={chosenRecipe.img} alt="" />
                    <h5>{chosenRecipe.chef}</h5>
                    <h6>{chosenRecipe.category}</h6>
                </section>
                <article className={styles.ingredients}>
                    {ingredients.map((ingredient, id) => (
                        <article key={id} className={styles.allMeasurements}>
                            <p>{ingredient.name}</p>
                            <span>{ingredient.amount * portionCounter}</span>
                            <p>{ingredient.measurement}</p>
                        </article>
                    ))}
                </article>
                <article className={styles.portionCount}>
                    <button className={(portionCounter === 1) ? `${styles.hideBtn}` : null} onClick={() => dispatch(decreasePortion())} >-</button>
                    <h5>{portionCounter}</h5>
                    <button onClick={() => dispatch(addPortion())}>+</button>
                </article>


                <h3 className={styles.descriptionTitle}>Såhär gör du</h3>
